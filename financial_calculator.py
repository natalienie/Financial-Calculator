...
class PensionPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Label(self, text='How much u want to withdraw per year after retire? ').grid(row=2)
        ttk.Label(self, text='How many years u anticipate to withdraw pension?').grid(row=3)
        ttk.Label(self, text='How many years left before you retire?').grid(row=4)
        ttk.Label(self, text='What is the interest rate now?').grid(row=5)
        ttk.Label(self, text='What is the inflation rate?').grid(row=6)
        ttk.Label(self, text='The amount you need to save per year staring now').grid(row=7)
        ttk.Label(self, text='Would u take consideration of inflation rate after retirement?').grid(row=8)
...
# ordinary annuatity or the other
# seperate two conditions
"""
def FV(principle, interest, pmt, i, ordinary=True):
    Sum1 = 0
    if ordinary:

        for i in range(0, i):
            Sum1 += pmt * ((1+interest)**i)
    else:
        for i in range(1, i +1):
            Sum1 += pmt * ((1+interest)**i)
    Sum2 = principle * (1+interest)**i

    return Sum1 + Sum2


def get_present_value(future_value, period, pmt, interest):

    sum1 = 0
    for i in range(1, period+1):
        sum1 += pmt / (1+interest) ** i
        sum2 = future_value / ((1 + interest) ** period)

    return sum1 + sum2


def find_hmy(principle, interest, pmt, target):
...
def find_interest(principle, pmt, period):



def find_pmt(principl, interest, target, period):


# write 2 more equations to get the NPER and Interest rate
"""
